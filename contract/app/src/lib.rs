...
fn upsert_agent(
    ...
    let existing = state.agents.get(&address).cloned();
    ...
    state.agents.insert(address, dna);
    for spec in specializations {
        let list = state.rankings.entry(spec.clone()).or_default();
        if !list.contains(&address) {
            list.push(address);
        }
        resort_category(state, &spec);
    }
    true
}
...
impl<'a> OnyxService<'a> {
    ...
    pub fn register_agent(&mut self, name: String, specializations: Vec<String>) -> bool {
        ...
        upsert_agent(&mut st, caller, name, specializations)
    }
    ...
    pub fn register_agent_for(
        ...
        if msg::source() != st.owner {
            panic!("Onyx: not owner");
        }
        upsert_agent(&mut st, address, name, specializations)
    }
    ...
    pub fn route_intent(&mut self, intent_id: u64) -> RoutingResult {
        ...
        let candidates = st
            .rankings
            .get(&category)
            .expect("Onyx: no agents for this category")
            .clone();
        let (best_agent, best_score) = candidates
            .iter()
...
impl<'a> QueryService<'a> {
    ...
    #[export]
    pub fn get_intent_count(&self) -> u64 {
        self.state.borrow().intent_counter
    }

    #[export]
    pub fn get_total_routings(&self) -> u64 {
        self.state.borrow().total_routings
    }
    ...
    pub fn get_top_agents(&self, limit: u32) -> Vec<RankEntry> {
        let st = self.state.borrow();
        let mut all: Vec<RankEntry> = st
            .agents
            .iter()
            .map(|(addr, dna)| RankEntry {
                agent: *addr,
                name: dna.name.clone(),
                weighted_score: dna.weighted_score,
                call_count: dna.call_count,
...
impl<'a> AdminService<'a> {
    ...
    pub fn remove_agent(&mut self, address: ActorId) -> bool {
        ...
        if st.agents.remove(&address).is_none() {
            return false;
        }
        for list in st.rankings.values_mut() {
